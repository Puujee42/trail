          ...
          <div className="flex-1 flex gap-1 overflow-x-auto w-full md:w-auto scrollbar-hide p-1">
            {categories.map((cat) => (
              <button
                ...
                className={`relative px-5 py-2.5 rounded-full text-sm font-bold whitespace-nowrap transition-colors flex-shrink-0 ${activeTab === cat.id ? "text-white" : "text-slate-500 hover:bg-slate-100"
                  }`}
              >
                {activeTab === cat.id && (
                  <motion.div
          ...
          <div>
            ...
            {oldPriceValue && (
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-0.5">
                  Save {Math.round(((oldPriceValue - priceValue) / oldPriceValue) * 100)}%
                </span>
            ...
            <span className={`text-xl font-black ${oldPriceValue ? 'text-rose-600' : 'text-slate-900'}`}>
