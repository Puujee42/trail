  ...
  return (
    <>
      ...
      <motion.header
        ...
        className={`hidden md:flex fixed top-8 inset-x-0 mx-auto w-fit min-w-[70%] rounded-full transition-all duration-500 z-50 backdrop-blur-xl items-center justify-between hover:bg-white px-8 ${
          isScrolled 
  ...
  return (
    <div className="relative" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
      <button 
        ...
        aria-expanded={open}
        className={`flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-full transition-all duration-300 ${
        mobile 
          ...
          >
            {LANGUAGES.map((lang) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 text-[13px] font-semibold flex items-center justify-between group transition-all ${
                  language === lang.code ? "bg-sky-50 text-sky-700" : "text-slate-600 hover:bg-slate-50"
                }`}
              >
